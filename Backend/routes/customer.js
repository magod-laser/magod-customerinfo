...
const { createFolder, copyallfiles, renameFolderIfExists } = require("../helpers/folderhelper");
const { misQuery, setupQuery, setupQueryMod, misQueryMod } = require("../helpers/dbconn");
...
customerRouter.post("/allcustomers", async (req, res, next) => {
  try {
    ...
    misQueryMod(`Select *, p.CreditDays from magodmis.cust_data c
...
customerRouter.post("/findcustcodebyname", async (req, res, next) => {
  console.log("findcustcodebyname");
...
customerRouter.post("/customer", async (req, res, next) => {
  ...
  try {
    ...
    let msg = "";
    //   console.log("customer - Yes - 1");
    // Check if the customer already exists
    misQueryMod(`SELECT * FROM magodmis.cust_data WHERE LOWER(Cust_name) = LOWER("${customerName}")`, (err, result) => {
      if (err) {
        logger.error(err);
        return res.status(500).send('Error checking customer data');
      }
      //        console.log("customer - Yes - 2");
      if (result.length > 0) {
        //          console.log("Customer Already Exists.." + customerName);
        misQueryMod(`UPDATE magodmis.cust_data SET IsBranch = "${isBranch}", Branch = "${branchName}" WHERE Cust_name = "${customerName}"
             AND Cust_Code = "${result[0].Cust_Code}"`, (err, result1) => {
          if (err) {
            logger.error(err);
        ...
        misQueryMod(
          ...
          async (err, runningno) => {
            ...
            let qno = (parseInt(runningno[0]["Running_No"]) + 1).toString().padStart(4, "0");
            let dwgfolder = qno;
            console.log("Dwg Folder : ", dwgfolder);
            createFolder("Customer", qno, dwgfolder, newQno, month, (err, fres) => {
              if (err) logger.error(err);
            });

            misQueryMod(`INSERT INTO magodmis.cust_data (Cust_Code, IsBranch, Cust_name, Branch, DwgLoc) VALUES ("${qno}", "${isBranch}", "${customerName}", "${branchName}", "${qno}")`
              , (err, ins) => {
                ...
                if (ins && ins.affectedRows > 0) {
                  misQueryMod(
                    ...
                    (err, updatedrunning) => {
                      if (err) {
                        logger.error(err);
                        return res.status(500).send('Error updating running number');
                      }
  ...
  try {
    ...
    misQueryMod(`SELECT * FROM magodmis.cust_data WHERE Cust_Code = '${custcode}'`, async (err, response) => {
      ...
      if (response && response.length > 0) {
        ...
        console.log("updateQuery-30012025", updateQuery);
...
customerRouter.post("/customerinsassembly", async (req, res, next) => {
  ...
  try {
    ...
    misQueryMod(
      ...
      (err, data) => {
        if (data.length > 0) {
          misQueryMod(`UPDATE magodmis.cust_assy_data SET AssyDescription='${assydescription}',MtrlCost=${mtrlcost},JobWorkCost=${jwcost},
            Status='Edit',Operation='${Operation}', Material='${Material}' 
            WHERE Cust_code = '${custcode}' and AssyCust_PartId = '${assycustpartid}'`, (err, updata) => {
            console.log("updated Assebly data");
          });
        } else {
...
//console.log("Checking Duplicate Customer Assembly data");
  try {
    ...
    if (assycustpartid != null) {
      ...
      //    console.log(data);
          if (data.length > 0) {
            //  misQueryMod(`Update magodmis.cust_assy_data Set `, (err, updata) => { })
...
customerRouter.post("/bomassemblyparts", async (req, res) => {
  ...
  try {
    ...
    console.log("Customer Code : ", req.body.custcode);
    console.log("dataarray : ", req.body.dataarray)
    let retdata = [];
    let assyprtid = 0;
    let bompartid = 0;
    for (let i = 0; i < dataarray.length; i++) {
      ...
      if (
        dataarray[i].partdesc !== "" && (dataarray[i].partid !== "" &&
          dataarray[i].partid !== null && dataarray[i].partid !== undefined)
      ) {


        misQueryMod(`Select assytbl.Id as assyid from magodmis.cust_assy_data assytbl 
          where AssyCust_PartId = '${dataarray[i].assyPartId}' and Cust_code='${ccode}'`, (err, assydata) => {

          assyprtid = assydata.assyId;
        })
        misQueryMod(`SELECT bomlist.Id as custbompartid FROM magodmis.cust_bomlist as bomlist 
            where PartId = '${dataarray[i].partid}' and Cust_code='${ccode}'`, (err, bompart) => {
              ...
              (err, massemid) => {
                misQueryMod(
                  `Update magodmis.cust_assy_bom_list SET Quantity= ${dataarray[i].qty} where Cust_AssyId = '${massemid[0].assyid}' 
                     And Cust_BOM_ListId = (SELECT bomlist.Id FROM magodmis.cust_bomlist as bomlist 
                      where PartId = '${dataarray[i].partid}' and Cust_code='${ccode}')`, (err, data) => {
                  if (err) logger.error(err);
                  retdata.push(data);
                });
                // }
  ...
  try {
    ...
    misQueryMod(
      ...
      (err, data1) => {
        ...
        if (data1.length > 0) {
          misQueryMod(`Update magodmis.cust_assy_data set AssyCust_PartId='${assemblyid}', Status='${asmstatus}',AssyDescription = '${asmdesc}',
            MtrlCost='${mtrlcost}',JobWorkCost='${jbwrkcost}',Operation='${operation}',Material='${material}' where MagodCode='${mmagodid}'`, (err, bomasmdata) => {
            if (err) logger.error(err);
            console.log(data1[0].Cust_Code)
            misQueryMod(`Select * from magodmis.cust_assy_data where Cust_Code = '${data1[0].Cust_Code}'`, (err, data) => {
...
customerRouter.post(`/deletebomassmparts`, async (req, res, next) => {
  ...
  try {
    ...
    //     misQueryMod(
    //       `Select assytbl.Id as assyid,Cust_Code from magodmis.cust_assy_data assytbl where AssyCust_PartId = '${asmpart}'`,  //'${asmid}'`,
    //       (err, asmdata) => {
    //         if (err) {
    //           logger.error(err);
    //           if (!responseSent) {
    ...
    // }
    console.log(ccode);
    console.log(selectedbomdata.assyPartId);
    console.log(selectedbomdata.partid);

    misQueryMod(`Select cad.Id as custassyid from magodmis.cust_assy_data cad where cad.AssyCust_PartId = '${selectedbomdata.assyPartId}'`, (err, asmdata) => {
      if (err) {
        logger.error(err);
        if (!responseSent) {
          res.send({ status: "error" }); // Send error response
      ...
      misQueryMod(`SELECT cb.Id as custbomId FROM magodmis.cust_bomlist as cb where PartId = '${selectedbomdata.partid}' and Cust_code='${ccode}'`, (err, asmprtdata) => {
        if (err) {
          logger.error(err);
          if (!responseSent) {
            res.send({ status: "error" }); // Send error response
...
customerRouter.post(`/customerduelist`, async (req, res, next) => {
  ...
  try {
    ...
    if (!custcode) return res.send(createError.BadRequest());
    //DATE_FORMAT(d.PaymentDate,"%d/%m%/%Y") as 'PaymentDate',
    misQueryMod(
...
customerRouter.post(`/customeroverduelist`, async (req, res, next) => {
  console.log("customeroverduelist : ", req.body);
  try {
    const custcode = req.body.custcode;
    const custcr = req.body.crdays;
    console.log("cr Days: ", custcr);
    if (!custcode) return res.send(createError.BadRequest());
...
customerRouter.post(`/pprcustomerduelist`, async (req, res, next) => {
  console.log("PPR Due List");
  console.log(req.body);
  try {
    const custcode = req.body.custcode;
    if (!custcode) return res.send(createError.BadRequest());
    //d.GrandTotal > d.PymtAmtRecd
    misQueryMod(
...
customerRouter.post(`/paymentterms`, async (req, res, next) => {
  console.log("Payment Terms ", req.body.crterms);
  try {
...
customerRouter.post('/duegenerate-pdf', async (req, res) => {
  ...
  const filePath = path.join(process.env.FILE_SERVER_PDF_PATH + custcode, fileName);

  let magodlogo = "../MagodLogo.png"; // "../../../../../../../MagodLogo.png"


  await misQueryMod('SELECT * FROM magod_setup.magodlaser_units', (err, unitData) => {
    //   console.log("custcode : ", custcode);
    misQueryMod(`SELECT DateDiff(Curdate(),d.Inv_Date) as DueDays,d.DC_Inv_No,d.IsDC,d.ScheduleId,DATE_FORMAT(d.Dc_inv_Date, "%d/%l/%Y") AS 'Dc_inv_Date' ,d.DC_InvType,
        ...
        ORDER BY d.Inv_Date`, (err, newData) => {
      ...
      if (newData.length > 0) {
        ...
        doc.fontSize(12).text(` ${unitData[0].RegisteredName}`, { align: 'center', textDecoration: 'bold' });
        ...
        doc.fontSize(9).text(`List of Invoices Due For Payment as on: ${ason}`, { align: 'center' });
        ...
        doc.text(`Customer Name: ${newData[0].Cust_Name}`, leftX, initialtextY, { align: 'left' });
        doc.text(`Due Amount: ${parseFloat(OverDAmt).toFixed(2)}`, rightX, initialtextY, { align: 'right' });
        doc.moveDown(); // Add some space before the table
        // Add table headers 
        doc.fontSize(9);
        const tableTop = 100;
        const itemSpacing = 25;
        const headers = ["Srl", "Inv No", "Inv Date", "PO No", "Amount", "Received", "Balance", "Due Date", "Due Days"];
        ...
        function printTableHeaders(doc, headerX, tableTop) {
          ...
          doc.moveTo(20, tableTop - 5).lineTo(600, tableTop - 5).stroke();
          headers.forEach((header, i) => {
            doc.text(header, headerX[i], tableTop);
          });
          doc.moveTo(20, tableTop + 15).lineTo(600, tableTop + 15).stroke();
          //   doc.font('Helvetica');
        }
        ...
        newData.forEach((item, i) => {
          ...
          // doc.text(item.InvTotal, headerX[4], y);
          // doc.text(item.PymtAmtRecd, headerX[5], y);
          // doc.text(item.Balance, headerX[6], y);

          doc.text(item.InvTotal.toString(), headerX[4], y, { align: 'right', width: 50 }); // Adjust width as needed 
          doc.text(item.PymtAmtRecd.toString(), headerX[5], y, { align: 'right', width: 50 }); // Adjust width as needed 
          doc.text(item.Balance.toString(), headerX[6], y, { align: 'right', width: 50 }); // Adjust width as needed
          doc.text(moment(item.DespatchDate).format('DD-MM-YYYY'), headerX[7], y);
...
function addPageNumbers(doc) {
  ...
  for (let i = 0; i < range.count; i++) {
    doc.switchToPage(i);
    doc.fontSize(8).text(`Page ${i + 1} of ${range.count}`, 300, doc.page.height - 30, {
      align: 'center'
    });
  }
}
...
customerRouter.post('/osgenerate-pdf', async (req, res) => {
  ...
  function printOSTableHeaders(doc, headerX, tableTop) {
    const headers = ["Inv No", "Inv Date", "Amount", "Received", "Balance", "Due Days", "PO No"];
    //  doc.font('Helvetica-Bold');
    doc.moveTo(20, tableTop + 5).lineTo(600, tableTop + 5).stroke();
    headers.forEach((header, i) => {
      doc.text(header, headerX[i], tableTop);
  ...
  await misQueryMod('SELECT * FROM magod_setup.magodlaser_units', async (err, unitData) => {
    if (err) logger.error(err);
    await misQueryMod(
      ...
      (err, newData) => {
        ...
        doc.fontSize(9).text(`Outstanding Invoices Report For ${req.body.custname}`, { align: 'center' });
        ...
        function printOSTableHeaders(doc, headerX, tableTop) {
          //   doc.font('Helvetica-Bold');
          doc.moveTo(30, tableTop - 1).lineTo(560, tableTop - 1).stroke();
          headers.forEach((header, i) => {
        ...
        newData.forEach((item, i) => {
          ...
          // moment(item.Inv_Date, 'DD/MM/YYYY');

          doc.text(moment(item.Inv_Date).format('DD/MM/YYYY'), headerX[1], y);
          doc.text(item.GrandTotal, headerX[2], y, { align: 'right', width: 50 });
          doc.text(item.PymtAmtRecd, headerX[3], y, { align: 'right', width: 50 });
          doc.text(item.Due, headerX[4], y, { align: 'right', width: 50 });
          //  doc.text(new Date(item.DespatchDate).toLocaleDateString(), headerX[7], y); 
          doc.text(item.DueDays, headerX[5], y, { align: 'right', width: 40 });
